...
class Mega_Menu_Walker_ACF extends Walker_Nav_Menu {
    ...
    if ($image_field) {
        ...
        $output .= '<div class="mega-menu-image">
                        <img id="mega-menu-dynamic-img" src="' . esc_url($image_url) . '" alt="' . esc_attr($item->title) . '" data-default="' . esc_url($image_url) . '">
                    </div>';
    }
   }
    ...
    // Inject product-family taxonomy terms after the menu item with class 'inject-product-family'
    if (in_array('inject-product-family', (array) $item->classes)) {
        $terms = get_terms([
        ...
        if (!is_wp_error($terms) && !empty($terms)) {
            foreach ($terms as $term) {
                $term_url  = get_term_link($term, 'product-family');
                $term_link = is_wp_error($term_url) ? '#' : esc_url($term_url);

                // Get ACF pf_menu_img for this term
                $term_image = get_field('pf_menu_img', 'product-family_' . $term->term_id);
                $term_image_url = '';
                if ($term_image) {
                    if (is_array($term_image)) {
                        $term_image_url = $term_image['url'];
                    } elseif (is_numeric($term_image)) {
                        $term_image_url = wp_get_attachment_url($term_image);
                    } else {
                        $term_image_url = $term_image;
                    }
                }

                $output .= '<li class="menu-item product-family-term" data-image="' . esc_url($term_image_url) . '">';
                $output .= '<a href="' . $term_link . '">' . esc_html($term->name) . '</a>';
                $output .= "</li>\n";
            }
        }
    }
